package Tipclass;
 ...
 class MyClass {
        ...
        /*Методы доступа к переменной alpha. Элементы класса могут
        обращаться к закрыты элементам того же класса*/
        void setAlpha(int а) {
            alpha = а;
        }
        int getAlpha() {
...
//Следующие обращения вполне допустимы, так как переменные beta и gamma являются открытыми
            ob.beta = 88;//Возможно, поскольку это открытые переменные
            ob.gamma = 99;
